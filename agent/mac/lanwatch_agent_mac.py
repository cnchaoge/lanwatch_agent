...
def report_heartbeat(agent_id, events=None, last_diag=None):
    """网络恢复时上报心跳（带离线诊断和本地事件）"""
    try:
        payload = {
            "events": events or [],
            "last_diag": last_diag,
        ...
        req = urllib.request.Request(
            SERVER_URL + "/api/" + agent_id + "/heartbeat",
...
def main():
    ...
    _prev_is_online = True   # 追踪前一轮状态，用于检测离线/恢复跳变
    ...
    while True:
        try:
            ...
            # ── 离线检测：网络断开时立即通知服务端 ──
            if _prev_is_online and not is_online:
                log.warning("[离线] 网关不可达，立即通知服务端")
                report_offline(agent_id)

            # ── 恢复检测：网络恢复时立即上报心跳 ──
            if not _prev_is_online and is_online:
                log.info("[恢复] 网络恢复，上报心跳")
                report_heartbeat(agent_id)

            _prev_is_online = is_online
